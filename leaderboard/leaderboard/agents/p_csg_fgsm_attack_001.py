...
class PCSGAgent(autonomous_agent.AutonomousAgent):
	...
	def run_step(self, input_data, timestamp):
		...
		if self.step < self.config.seq_len:
			rgb_front = torch.from_numpy(scale_and_crop_image(Image.fromarray(tick_data['rgb']), scale=1.0, crop=[160, 300])).unsqueeze(0)
			rgb_left = torch.from_numpy(scale_and_crop_image(Image.fromarray(tick_data['rgb_left']), scale=0.8, crop=[160, 234], shift_x=0, shift_y=-47)).unsqueeze(0)
			rgb_right = torch.from_numpy(scale_and_crop_image(Image.fromarray(tick_data['rgb_right']), scale=0.8, crop=[160, 234], shift_x=0, shift_y=47)).unsqueeze(0)
			rgb = torch.cat((rgb_left, rgb_front, rgb_right), axis=-1)
		...
		if(self.step  % 2 == 0 or self.step <= 4):
			...
			pred_wp = self.net(image, self.lidar_processed[0], target_point, gt_measurements)[0]
			target_wp = torch.tensor([[-1.0,-2.0],[-2.0,-4.0],[-3.0,-6.0],[-4.0,-8.0]])[None,...].to(image.device)
			loss = self.net.wp_loss(pred_wp,target_wp)
			loss.backward(retain_graph=True)

			image_grad = image.grad.data

			perturbed_rgb = fgsm_attack_rgb(image,0.01,image_grad)


			unified[0] = perturbed_rgb
...
def fgsm_attack_rgb(image, epsilon, data_grad):
    ...
    perturbed_image = image + epsilon*sign_data_grad * 255.0
