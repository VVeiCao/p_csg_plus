...
routes["training_routes/routes_town01_long.xml"] = "scenarios/town01_all_scenarios.json"
...
routes["training_routes/routes_town02_long.xml"] = "scenarios/town02_all_scenarios.json"
...
routes["training_routes/routes_town03_long.xml"] = "scenarios/town03_all_scenarios.json"
...
routes["training_routes/routes_town04_long.xml"] = "scenarios/town04_all_scenarios.json"
...
routes["training_routes/routes_town06_long.xml"] = "scenarios/town06_all_scenarios.json"
...
routes["training_routes/routes_town07_long.xml"] = "scenarios/town07_all_scenarios.json"
...
routes["training_routes/routes_town10_long.xml"] = "scenarios/town10_all_scenarios.json"

ip_ports = []

for port in range(20000, 20015, 2):
...
configs.append("weather-mixed.yaml")
configs.append("clear-weather.yaml")
...
def generate_script(
    ...
    lines.append("\n")
    base = open("base_script_new.sh").readlines()
...
if not os.path.exists("bashs"):
    os.mkdir("bashs")
if not os.path.exists("bashs/weather-mixed"):
    os.mkdir("bashs/weather-mixed")
if not os.path.exists("bashs/clear-weather"):
    os.mkdir("bashs/clear-weather")

for i, route in enumerate(routes):
    ip, port, tm_port = ip_ports[i//3]
    ...
    fw = open(
        "bashs/weather-mixed/%s.sh" % (route.split("/")[1].split(".")[0]), "w"
    )
    for line in script:
        fw.write(line)
    ...
    fw = open(
        "bashs/clear-weather/%s.sh" % (route.split("/")[1].split(".")[0]), "w"
    )
    for line in script:
        fw.write(line)