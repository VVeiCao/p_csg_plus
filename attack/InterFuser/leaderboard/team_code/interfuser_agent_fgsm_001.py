...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.epsilon = 0.01 # [0.0, .05, .1 , .2, .3]
    ...
    def run_step(self, input_data, timestamp):
        ...
        rgb_left.requires_grad=True
        rgb_right.requires_grad=True
        rgb_center.requires_grad=True
        ...
        "stop_cls": torch.nn.CrossEntropyLoss(),
        }

        for paramter in self.net.parameters():
            paramter.requires_grad=False
        
        # self.net.zero_grad()

        output = self.net(input_data)
        target_wp = torch.tensor([[-1.0,-2.0],[-2.0,-4.0],[-3.0,-6.0],[-4.0,-8.0],[-5.0,-10.0],
                                    [-6.0,-12.0],[-7.0,-14.0],[-8.0,-16.0],[-9.0,-18.0],[-10.0,-20.0]])[None,...].to(output[1].device)
        loss_waypoints = loss_fns["waypoints"](output[1], target_wp)


        loss_waypoints.backward(retain_graph=True)

        data_grad_rgb = input_data["rgb"].grad.data
        data_grad_rgb_left = input_data["rgb_left"].grad.data
        data_grad_rgb_right = input_data["rgb_right"].grad.data
        data_grad_rgb_center = input_data["rgb_center"].grad.data


        rgb_inv = inv_normalize(input_data["rgb"])
        rgb_left_inv = inv_normalize(input_data["rgb_left"])
        rgb_right_inv = inv_normalize(input_data["rgb_right"])
        rgb_center_inv = inv_normalize(input_data["rgb_center"])



        epsilon = self.epsilon
        perturbed_rgb = fgsm_attack_rgb(rgb_inv, epsilon, data_grad_rgb)
        perturbed_rgb_left = fgsm_attack_rgb(rgb_left_inv, epsilon, data_grad_rgb_left)
        perturbed_rgb_right = fgsm_attack_rgb(rgb_right_inv, epsilon, data_grad_rgb_right)
        perturbed_rgb_center = fgsm_attack_rgb(rgb_center_inv, epsilon, data_grad_rgb_center)


        input_data["rgb"] = normalize(perturbed_rgb)
        input_data["rgb_left"] = normalize(perturbed_rgb_left)
        input_data["rgb_right"] = normalize(perturbed_rgb_right)
        input_data["rgb_center"] = normalize(perturbed_rgb_center)
...
# FGSM attack code
def fgsm_attack_rgb(image, epsilon, data_grad):
    # Collect the element-wise sign of the data gradient
    sign_data_grad = data_grad.sign()
    # Create the perturbed image by adjusting each pixel of the input image
    perturbed_image = image + epsilon*sign_data_grad
    # Adding clipping to maintain [0,256] range
