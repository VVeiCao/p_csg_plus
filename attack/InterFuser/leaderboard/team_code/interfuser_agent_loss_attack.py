...
class DisplayInterface(object):
    ...
    def run_interface(self, input_data):
        ...
        surface[:150, 325:475] = input_data['r=gb_focus']
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    ...
    def run_step(self, input_data, timestamp):
        ...
        # attack model
        attack_model_front_dict = torch.load('./logs/loss_attack_interfuser/30_front_ckpt.tar')
        attack_model_right_dict = torch.load('./logs/loss_attack_interfuser/30_right_ckpt.tar')
        attack_model_left_dict = torch.load('./logs/loss_attack_interfuser/30_left_ckpt.tar')
        attack_model_front_center_dict = torch.load('./logs/loss_attack_interfuser/30_center_ckpt.tar')
...
#             blended = self._create_blended_img(blended, mask, normalized_color, self.alpha[idx] * 0.6)
#         return blended
...
def save_denorm_image(norm_image,dir):
    import torchvision.transforms as transforms
    inv_normalize = transforms.Normalize(
        mean=[-0.485/0.229, -0.456/0.224, -0.406/0.225],
        std=[1/0.229, 1/0.224, 1/0.225]
    )
    denorm_image = inv_normalize(norm_image)
    denorm_image = transforms.ToPILImage()(norm_image)
    denorm_image.save(dir)

save_denorm_image(perturbed_rgb[0],"image_output/rgb_fgsm_01.jpg")
save_denorm_image(perturbed_rgb_left[0],"image_output/rgb_left_fgsm_01.jpg")
save_denorm_image(perturbed_rgb_right[0],"image_output/rgb_right_fgsm_01.jpg")
save_denorm_image(perturbed_rgb_center[0],"image_output/rgb_center_fgsm_01.jpg")