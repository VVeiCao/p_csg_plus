...
class HybridAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file,route, route_index=None):
    ...
    def run_step(self, input_data, timestamp):
        ...
        # prepare image input
        image = self.prepare_image(tick_data) # 0-255
        ...
        # image_norm = composed(image)

        image.requires_grad=True

        for paramter in self.nets[0].parameters():
            paramter.requires_grad=False

        pred_wp, _ = self.nets[0].forward_ego(image, lidar_bev, target_point, target_point_image, velocity,
        ...
        loss = torch.mean(torch.abs(pred_wp - target_wp))
        ...
        output = image.squeeze(0).permute(1, 2, 0).cpu().detach().numpy()
        save_path_rgb = os.path.join("/home/ubuntu/projects/P_CSG/frames/transfuer/fgsm001", 'rgb', filename)
        Image.fromarray(np.uint8(output)).save(save_path_rgb)
