...
class HybridAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, route = None, route_index=None):
    ...
    def tick(self, input_data):
        ...
        for pos in ['left', 'front', 'right']:
            ...
            rgb.append(rgb_pos)
        rgb = np.concatenate(rgb, axis=1)# (160, 960, 3)
        ...
        # rgb = composed(rgb)
        # model_dict = torch.load('/home/slt/gitlab_file/transfuser+/transfusep_attack/logs/loss_attack_transfuserp/run_27/60_ckpt.tar')
        # model = ImageDot()
        # model.load_state_dict(model_dict)
        # rgb = model(rgb).squeeze(0)
        # inv_normalize = transforms.Normalize(
        #         mean=[-0.485/0.229, -0.456/0.224, -0.406/0.225],
        #         std=[1/0.229, 1/0.224, 1/0.225]
        #     )
        # rgb = inv_normalize(rgb)
        # rgb = rgb.permute(1,2,0).numpy() # unnormalized 
    ...
    def run_step(self, input_data, timestamp):
        ...
        # prepare image input
        image = self.prepare_image(tick_data) # ([1, 3, 160, 704])
        from torchvision import transforms
        # composed = transforms.Compose(
        #         [
        #         transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
        ...
        # rgb = composed(image)
        model_dict = torch.load('attack/transfuserp/dot_ckpt/65_ckpt.tar')
        model = ImageDot()
        model.load_state_dict(model_dict)
        rgb = model(image)
        inv_normalize = transforms.Normalize(
        ...
        image = inv_normalize(rgb)

        

        frame = self.step // 5
        filename = f"{self.route}_{frame:04d}.png"
        output = rgb.squeeze(0).permute(1, 2, 0).cpu().detach().numpy()
        save_path_rgb = os.path.join("/home/ubuntu/projects/P_CSG/frames/transfuer/dot", 'rgb', filename)
